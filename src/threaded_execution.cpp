...
void thread_A_start(core &c1, vector<process> &processes) {
    while (true) { // loop infinito
        ...
        //Enquanto (timestamp mod quantum != 0)
        // run the process for the quantum time
        
        while(c1.clock % c1.proc.quantum != 0)
        ...
        processes.push_back(processes[currentProcessIndex]);
        currentProcessIndex++;
        turnA = false;
        cv.notify_all();
    }
...
void thread_B_start(core &c2, vector<process> &processes) {
    while (true) {
        ...
        while (chrono::duration_cast<chrono::seconds>(chrono::steady_clock::now() - start).count() < c2.proc.quantum) {
            c2.running_asm();
        }
